...
def test_create_post_default_published_true(authorized_client, test_user,test_posts):
    res = authorized_client.post("/posts/", json={"title": "arbitary", "content": "hello"})
    created_post = schema.Post(**res.json())
    assert res.status_code == 201
...
def test_unauthorized_user_create_posts(client, test_posts,test_user):
    res = client.post("/posts/", json={"title": "arbitary", "content": "hello"})
    assert res.status_code == 401
...
def test_delete_post_non_exist(authorized_client,test_user,test_posts):
    res = authorized_client.delete(f"/posts/90000")
    assert res.status_code == 404
...
def test_update_post_non_exist(authorized_client,test_user,test_posts):
    ...
    res = authorized_client.put(f"/posts/90000", json=data)
    assert res.status_code == 404
